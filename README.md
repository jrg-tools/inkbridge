...
- Idle **light sleep** after 5 minutes on the main menu, waking instantly on either button. The header shows a small status icon (moon = eligible to nap here, lightning = USB connected) that stays in sync even without touching a button.
...
- **Idle light sleep**: after 5 minutes untouched on the main menu, the device drops into light sleep (`esp_light_sleep_start`) and wakes instantly on either button press. Scoped to the main menu only, so it never naps mid-config or mid-connect.
...
- **Script buttons**: each one configured in the web UI (label, HA script ID, icon) shows up as an icon-only square. Pressing it joins WiFi (trying each saved network in order), calls `script.turn_on` for that script's entity, and returns to the menu — a spinner is the only feedback while it works, and the header shows its name in the footer while it's selected.
...
src/
  ...
  components/               UITheme (fonts/metrics), Icons + icons/IconBitmaps.h, UiChrome (header/footer/rows)
  homeassistant/            HA REST client (fetch states, toggle, run scripts)
  network/                  ConfigWebServer (JSON API + static SvelteKit UI), WifiConnector (multi-network fallback)
...
Requires ImageMagick (`magick`) on `PATH` for SVG rasterization. Writes straight back into `src/components/icons/IconBitmaps.h` — don't hand-edit that file.
...
Settings are grouped as `transfer` (`wifiNetworks: [{ssid, password}]`, `haHost`, `haPort`, `haToken`, `haEntities`, `haScripts: [{label, id, icon}]`) and `settings` (`language`, `fontFamily`, `apSsid`, `apPassword`).
